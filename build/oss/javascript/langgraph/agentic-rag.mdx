...
* Grade retrieved document content for relevance to the question (`gradeDocuments`) and route to next step:
  ...
  * If relevant, proceed to `generate` and generate final response using the [`ToolMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages_tool.ToolMessage.html) with the retrieved document context
